...
parameters_segmentation['threshold']=230  # mm/h mixing ratio (until which threshold the area is taken into account)
...
for file in test_list:
    ...
    Features.to_hdf(os.path.join(savedir,'Features_' + str(i) + '_230.h5'),'table')
    ...
    # mask as xarray
    mask = xr.DataArray.from_iris(Mask)
    print('segmentation based on surface precipitation performed, start saving results to files',mask.dims)
    mask.to_netcdf(savedir+ 'mask_segmentation_test.nc')
    iris.save([Mask],os.path.join(savedir,'Mask_Segmentation_precip' + str(i) + '_230.nc'),zlib=True,complevel=4)                
