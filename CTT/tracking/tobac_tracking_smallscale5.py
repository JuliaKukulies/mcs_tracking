...
parameters_features['n_min_threshold']= 5 # minimum nr of contiguous pixels for thresholds, 10 pixels = ca. 2000 km2, 50 pixel ca. 10 000 km2
...
for f in file_list[61::]:
    ...
    Features.to_hdf(os.path.join(savedir,'smallscale5/Features_' + str(i) + '.h5'),'table')
    ...
    iris.save([Mask],os.path.join(savedir,'smallscale5/Mask_Segmentation_' + str(i) + '.nc'),zlib=True,complevel=4)                
    Features_Precip.to_hdf(os.path.join(savedir,'smallscale5/Features_cells_' + str(i) + '.h5'),'table')
