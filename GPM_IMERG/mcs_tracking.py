...
stats= ['label', 'date', 'time', 'lat', 'lon', 'PREC_mean','PREC_tot' ,'PREC_max', 'PREC_min', 'PREC_area', 'PREC_skew', 'PF_mean', 'PF_area', 'PF_tot', 'ELEV_mean', 'ELEV_max']
...
def __processing_files__(threshold_prec, threshold_area, threshold_timesteps,threshold_overlap,threshold_max_area,  stats, system_stats, working_dir, all_mcs_labels, files, t):
        ...
        if len(files[month]) == 0:
            ...
            for file in files[month]:
                if t  == 0: 
                    ...
                    if np.shape(mcs_labels[mcs_labels > 0])[0] > 0: # if MCS are present in this timestep
            ...
            system_stats = system_stats.sort_values(['label', 'date', 'time'] )
            #system_stats = size_con(all_mcs_labels, system_stats, threshold_max_area)
            system_stats = timestep_con(all_mcs_labels, system_stats, threshold_timesteps)
            output_path= working_dir + 'tracks/tracking_7mm6h20_noelev_'+   date[0:6]+ '_' + 'tracks.nc'
            create_netcdf(system_stats, output_path)    



# process files with elevation extraction
__processing_files__(threshold_prec, threshold_area, threshold_timesteps,threshold_overlap,threshold_max_area, stats, system_stats, working_dir, all_mcs_labels, files, t)
